...
fn test_type_of() {
    ...
    assert_eq!(
        engine
            .eval::<String>(r#"type_of([true, 2, "hello"])"#)
            .unwrap(),
        "array"
    );
    ...
    assert_eq!(
        engine
            .eval::<String>(r#"type_of(#{a:true, "":2, "z":"hello"})"#)
            .unwrap(),
        "map"
    );
    ...
    assert_eq!(
        engine.eval::<String>(r#"type_of("hello")"#).unwrap(),
        "string"
    );

    #[cfg(not(feature = "no_object"))]
    assert_eq!(
        engine.eval::<String>(r#""hello".type_of()"#).unwrap(),
        "string"
    );

    #[cfg(not(feature = "only_i32"))]
    assert_eq!(
        engine.eval::<String>("let x = 123; type_of(x)").unwrap(),
        "i64"
    );
