...
fn test_bit_fields() {
    ...
    assert_eq!(
        engine
            .eval::<INT>("let x = 10; x[0] = true; x[1] = false; x")
            .unwrap(),
        9
    );
    assert_eq!(
        engine.eval::<INT>("let x = 10; get_bits(x, 1, 3)").unwrap(),
        5
    );
    ...
    assert_eq!(
        engine
            .eval::<INT>("let x = 10; set_bits(x, 1, 3, 7); x")
            .unwrap(),
        14
    );
    ...
    #[cfg(not(feature = "only_i32"))]
    {
        assert_eq!(
            engine
                .eval::<INT>("let x = 255; get_bits(x, -60, 2)")
                .unwrap(),
            3
        );
        assert_eq!(
            engine
                .eval::<INT>("let x = 0; set_bits(x, -64, 1, 15); x")
                .unwrap(),
            1
        );
        assert_eq!(
            engine
                .eval::<INT>("let x = 0; set_bits(x, -60, 2, 15); x")
                .unwrap(),
            0b00110000
        );
    }
    assert_eq!(
        engine.eval::<INT>("let x = 10; x[1..4] = 7; x").unwrap(),
        14
    );
    assert_eq!(
        engine
